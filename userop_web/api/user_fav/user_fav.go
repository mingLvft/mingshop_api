...
import (
	...
	"go.uber.org/zap"
	"mingshop_api/userop_web/api"
	"mingshop_api/userop_web/forms"
...
func New(ctx *gin.Context) {
	...
	_, err := global.GoodsSrvClient.GetGoodsDetail(context.Background(), &proto.GoodInfoRequest{Id: int32(userFavForm.GoodsId)})
	if err != nil {
		zap.S().Errorw("商品不存在")
		api.HandleGrpcErrorToHttp(err, ctx)
		return
